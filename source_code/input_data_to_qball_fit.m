function input_data_to_qball_fit
    ...
    sh_dwi_reg_1 = zeros(size(req_4d_dwmri));
        ...
        for j = 1:size(req_4d_dwmri,2)
            for k = 1:size(req_4d_dwmri,3)
                if mask(i,j,k)
                    ...
                    % Reconstruct signal with spherical harmonic coefficients using least squared fit                   
                    D = (basis'*basis)\basis'*squeeze(req_4d_dwmri(i,j,k,:));                  
                    ...
                    % Reconstruct signal with spherical harmonic coefficients using least squared fit                   
                    E = (basis'*basis + L)\basis'*squeeze(req_4d_dwmri(i,j,k,:));                  
                    
                    % Reconstruct signal
                    sh_signal = basis*E;
                    
                    % Store it
                    sh_dwi_reg_1(i,j,k,:) = sh_signal;
